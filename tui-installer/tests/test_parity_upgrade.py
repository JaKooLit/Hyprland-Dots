...
def test_upgrade_restores_hypr_assets_and_configs(
    fake_home,
    monkeypatch: pytest.MonkeyPatch,
    prompt_replace_yes,
    prompt_confirm_yes,
) -> None:
    """Upgrade: asserts hypr assets restore + UserConfigs restore prompts path."""
    ...
    # Seed existing install to force backups
    existing_hypr = fake_home.config / "hypr"
    write_text(existing_hypr / "v2.3.19", "")
    write_text(
        existing_hypr / "monitors.conf", "monitor = eDP-1, 1920x1080@60,auto,1\n"
    )
    write_text(existing_hypr / "animations" / "a.conf", "anim\n")
    write_text(existing_hypr / "Monitor_Profiles" / "p.conf", "prof\n")
    write_text(existing_hypr / "wallpaper_effects" / "w.conf", "wall\n")

    # Provide UserConfigs backup content (post-backup it becomes backup-hypr/...)
    write_text(existing_hypr / "UserConfigs" / "UserSettings.conf", "custom\n")
    write_text(existing_hypr / "UserConfigs" / "Startup_Apps.conf", "exec-once = foo\n")
    ...
    asyncio.run(
        orch.run_install(
            cfg,
            log=log,
            log_file=fake_home.copy_logs / "upgrade-test.log",
    ...
    # Assert: backup exists and assets were merged/restored.
    backups = sorted(fake_home.config.glob("hypr-backup*"))
    assert backups, "Expected hypr backup directory"
    ...
    # Assert: UserConfigs restore path ran (prompt-confirm yes means it should merge)
    # Presence of backed-up UserSettings.conf under new tree indicates merge_tree occurred.
    assert (fake_home.config / "hypr" / "UserConfigs").is_dir()
    assert "custom" in read_text(

...
Rectangle { // Window
    ...
    property real initX: Math.max((windowData?.at[0] - monitorData?.reserved[0] - monitorData?.x) * (root.scale / monitorData.scale), 0) + xOffset
    property real initY: Math.max((windowData?.at[1] - monitorData?.reserved[1] - monitorData?.y) * (root.scale / monitorData.scale), 0) + yOffset
    property real xOffset: 0
    property real yOffset: 0
    
    property var targetWindowWidth: windowData?.size[0] * (root.scale / monitorData.scale)
    property var targetWindowHeight: windowData?.size[1] * (root.scale / monitorData.scale)
    ...
    x: initX
    y: initY
    width: Math.min(windowData?.size[0] * (root.scale / monitorData.scale), (restrictToWorkspace ? windowData?.size[0] : availableWorkspaceWidth - x + xOffset))
    height: Math.min(windowData?.size[1] * (root.scale / monitorData.scale), (restrictToWorkspace ? windowData?.size[1] : availableWorkspaceHeight - y + yOffset))

    radius: Appearance.rounding.windowRounding * (root.scale / monitorData.scale)
